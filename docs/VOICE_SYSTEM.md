...
- **Track-level deafen guard**: Remote microphone `MediaStreamTrack` playback is disabled locally while deafened and the policy is re-applied on subscribe, sender mute/unmute, remote stream refresh, and LiveKit reconnect. Screen-share audio tracks remain enabled and continue to follow their independent viewer controls.
