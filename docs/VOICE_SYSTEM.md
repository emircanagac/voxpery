...
- **Track-level deafen guard**: Remote microphone `MediaStreamTrack` playback is disabled locally while deafened and the policy is re-applied on subscribe, LiveKit `TrackMuted`/`TrackUnmuted`, remote stream refresh, and reconnect. This is applied after LiveKit updates the underlying track's enabled state, while screen-share audio remains independent.
