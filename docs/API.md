...
- `POST /api/servers/:server_id/leave`
  - Returns `403` for the official Voxpery community (identified by its reserved `voxpery` invite code); members can leave other servers as before. Unbanned accounts that left the official community before this restriction rejoin on their next login.
