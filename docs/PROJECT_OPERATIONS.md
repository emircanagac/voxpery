...
- **Security monitoring gates**: CodeQL scans only the languages affected by a PR (server/desktop Rust, web or CI-script JavaScript, and GitHub Actions workflows), while weekly and manual runs scan all languages. Dependency security audits run separately. Review their output before release; an upstream-blocked advisory needs an explicit release decision rather than being silently ignored.
