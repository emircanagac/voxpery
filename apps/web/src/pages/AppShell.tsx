...
export default function AppShell() {
  ...
  useEffect(() => {
    ...
    const valid = last === 'online' || last === 'idle' || last === 'dnd' || last === 'offline'
    if (!valid) {
    ...
    statusRestoredRef.current = true
    authApi.updateStatus(last as 'online' | 'idle' | 'dnd' | 'offline', token ?? null).then(
  ...
  useEffect(() => {
    const unsub = subscribe((evt: unknown) => {
      try {
        const e = evt as { type?: string; data?: { user?: any; user_id?: string; channel_id?: string | null; server_id?: string | null; status?: string; muted?: boolean; deafened?: boolean; screen_sharing?: boolean; message?: { author?: { user_id?: string } } } }
        ...
        if (e?.type === 'VoiceControlUpdate') {
          const { user_id, muted, deafened, screen_sharing } = e.data ?? {}
          if (user_id) setVoiceControl(user_id, !!muted, !!deafened, !!screen_sharing)
        }
        ...
        if (e?.type === 'NewMessage') {
          ...
          const isDmRoute = location.pathname === '/app/dm' || location.pathname.startsWith('/app/dm/')
    ...
    <div className={`shell-layout${isFriendsOrDm ? ' shell-layout-social' : ''}`}>
      <header className="shell-topbar">
        <div className="shell-left">
          <button type="button" className="shell-brand" onClick={() => navigate('/app/friends', { state: { tab: 'friends' } })}>
